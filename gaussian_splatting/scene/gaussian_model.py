...
class GaussianModel:
    ...
    ##
    def load_tensors(self, model_path):
        """
        Load Gaussian model tensors from a saved PyTorch file.
        
        This method loads pre-trained Gaussian parameters from a .pt file
        and initializes the model's internal tensors.
        
        Args:
            model_path (str): Path to the saved tensor file (.pt format)
        ...
        Example:
            ...
            >>> success = gaussian_model.load_tensors("../data/ckpts/optimized_params.pt")
            >>> if success:
            >>>     print(f"Loaded {gaussian_model._xyz.shape[0]} Gaussians")
        """
        try:
            # Load the tensor list from file
            model_loaded = torch.jit.load(model_path, map_location="cuda")
            ...
            for name, param in model_loaded.named_parameters():
                ...
                loaded_tensor_list.append(param.detach())

            # Extract individual tensors
            self._xyz = nn.Parameter(
                loaded_tensor_list[0].requires_grad_(True)
            )
            
            self._features_dc = loaded_tensor_list[1]
            # Ensure features_dc has the correct shape (N, 1, 3)
            if self._features_dc.dim() == 2:
            ...
            self._features_rest = nn.Parameter(
                loaded_tensor_list[2].requires_grad_(True)
            )
            self._opacity = nn.Parameter(
                loaded_tensor_list[3].requires_grad_(True)
            )
            self._scaling = nn.Parameter(
                loaded_tensor_list[4].requires_grad_(True)
            )
            self._rotation = nn.Parameter(
                loaded_tensor_list[5].requires_grad_(True)
            )
            ...
            self.unique_kfIDs = torch.zeros((num_points,)).int()
            self.n_obs = torch.zeros((num_points,)).int()
            
            print(f"Successfully loaded {num_points} Gaussians from {model_path}")
            return True
    ...
    def build_covariance_from_scaling_rotation(
        ...
        # print(" Rotation at 29181: ", rotation[29181])
        # print(" scaling at 29181: ", scaling[29181])
    ...
    def get_covariance(self, scaling_modifier=1):
        print('get_covariance called')
        return self.covariance_activation(
            self.get_scaling, scaling_modifier, self.rotation_activation(self._rotation)
        )
