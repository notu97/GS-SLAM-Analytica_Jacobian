	...
	float *dL_dtau)
{
	...
	if (denom2inv != 0)
	{
		...
		printf("dL_dcov2D idx %d: dL_da=%f dL_db=%f dL_dc=%f\n", idx, dL_da, dL_db, dL_dc);
	...
	// [CLAUDE CODE] DEBUG: Compute and print dCov2D_dtau (3x6 Jacobian: rows = a, b, c; cols = rho1..3, theta1..3)
	{
		float dcov2D_dtau[3][6];
		for (int comp = 0; comp < 3; comp++) {
			float seed_a = (comp == 0) ? 1.0f : 0.0f;
			float seed_b = (comp == 1) ? 1.0f : 0.0f;
			float seed_c = (comp == 2) ? 1.0f : 0.0f;

			// dT from unit seed on (a, b, c)
			float s_dT00 = 2 * (T[0][0] * Vrk[0][0] + T[0][1] * Vrk[0][1] + T[0][2] * Vrk[0][2]) * seed_a +
				(T[1][0] * Vrk[0][0] + T[1][1] * Vrk[0][1] + T[1][2] * Vrk[0][2]) * seed_b;
			float s_dT01 = 2 * (T[0][0] * Vrk[1][0] + T[0][1] * Vrk[1][1] + T[0][2] * Vrk[1][2]) * seed_a +
				(T[1][0] * Vrk[1][0] + T[1][1] * Vrk[1][1] + T[1][2] * Vrk[1][2]) * seed_b;
			float s_dT02 = 2 * (T[0][0] * Vrk[2][0] + T[0][1] * Vrk[2][1] + T[0][2] * Vrk[2][2]) * seed_a +
				(T[1][0] * Vrk[2][0] + T[1][1] * Vrk[2][1] + T[1][2] * Vrk[2][2]) * seed_b;
			float s_dT10 = 2 * (T[1][0] * Vrk[0][0] + T[1][1] * Vrk[0][1] + T[1][2] * Vrk[0][2]) * seed_c +
				(T[0][0] * Vrk[0][0] + T[0][1] * Vrk[0][1] + T[0][2] * Vrk[0][2]) * seed_b;
			float s_dT11 = 2 * (T[1][0] * Vrk[1][0] + T[1][1] * Vrk[1][1] + T[1][2] * Vrk[1][2]) * seed_c +
				(T[0][0] * Vrk[1][0] + T[0][1] * Vrk[1][1] + T[0][2] * Vrk[1][2]) * seed_b;
			float s_dT12 = 2 * (T[1][0] * Vrk[2][0] + T[1][1] * Vrk[2][1] + T[1][2] * Vrk[2][2]) * seed_c +
				(T[0][0] * Vrk[2][0] + T[0][1] * Vrk[2][1] + T[0][2] * Vrk[2][2]) * seed_b;

			// dJ from dT
			float s_dJ00 = W[0][0] * s_dT00 + W[0][1] * s_dT01 + W[0][2] * s_dT02;
			float s_dJ02 = W[2][0] * s_dT00 + W[2][1] * s_dT01 + W[2][2] * s_dT02;
			float s_dJ11 = W[1][0] * s_dT10 + W[1][1] * s_dT11 + W[1][2] * s_dT12;
			float s_dJ12 = W[2][0] * s_dT10 + W[2][1] * s_dT11 + W[2][2] * s_dT12;

			// dt{x,y,z} from dJ
			float s_dtx = x_grad_mul * -h_x * tz2 * s_dJ02;
			float s_dty = y_grad_mul * -h_y * tz2 * s_dJ12;
			float s_dtz = -h_x * tz2 * s_dJ00 - h_y * tz2 * s_dJ11 + (2 * h_x * t.x) * tz3 * s_dJ02 + (2 * h_y * t.y) * tz3 * s_dJ12;

			// SE3 path: dt → dtau (through camera-space point)
			for (int i = 0; i < 3; i++) {
				float3 cr = dpC_drho.cols[i];
				float3 ct = dpC_dtheta.cols[i];
				dcov2D_dtau[comp][i] = s_dtx * cr.x + s_dty * cr.y + s_dtz * cr.z;
				dcov2D_dtau[comp][i + 3] = s_dtx * ct.x + s_dty * ct.y + s_dtz * ct.z;
			}

			// W path: dT → dW → dtheta (through rotation matrix)
			float s_dW_data[9];
			s_dW_data[0] = J[0][0] * s_dT00;
			s_dW_data[3] = J[0][0] * s_dT01;
			s_dW_data[6] = J[0][0] * s_dT02;
			s_dW_data[1] = J[1][1] * s_dT10;
			s_dW_data[4] = J[1][1] * s_dT11;
			s_dW_data[7] = J[1][1] * s_dT12;
			s_dW_data[2] = J[0][2] * s_dT00 + J[1][2] * s_dT10;
			s_dW_data[5] = J[0][2] * s_dT01 + J[1][2] * s_dT11;
			s_dW_data[8] = J[0][2] * s_dT02 + J[1][2] * s_dT12;

			mat33 s_dW(s_dW_data);
			float3 s_dWc1 = s_dW.cols[0];
			float3 s_dWc2 = s_dW.cols[1];
			float3 s_dWc3 = s_dW.cols[2];

			float3 s_dtheta_w;
			s_dtheta_w.x = dot(s_dWc1, n_W1_x.cols[0]) + dot(s_dWc2, n_W2_x.cols[0]) + dot(s_dWc3, n_W3_x.cols[0]);
			s_dtheta_w.y = dot(s_dWc1, n_W1_x.cols[1]) + dot(s_dWc2, n_W2_x.cols[1]) + dot(s_dWc3, n_W3_x.cols[1]);
			s_dtheta_w.z = dot(s_dWc1, n_W1_x.cols[2]) + dot(s_dWc2, n_W2_x.cols[2]) + dot(s_dWc3, n_W3_x.cols[2]);

			dcov2D_dtau[comp][3] += s_dtheta_w.x;
			dcov2D_dtau[comp][4] += s_dtheta_w.y;
			dcov2D_dtau[comp][5] += s_dtheta_w.z;
		}

		printf("dCov2D_dtau idx %d:\n", idx);
		printf("dCov2D_dtau idx %d  da/dtau: %f %f %f %f %f %f\n", idx, dcov2D_dtau[0][0], dcov2D_dtau[0][1], dcov2D_dtau[0][2], dcov2D_dtau[0][3], dcov2D_dtau[0][4], dcov2D_dtau[0][5]);
		printf("dCov2D_dtau idx %d  db/dtau: %f %f %f %f %f %f\n", idx, dcov2D_dtau[1][0], dcov2D_dtau[1][1], dcov2D_dtau[1][2], dcov2D_dtau[1][3], dcov2D_dtau[1][4], dcov2D_dtau[1][5]);
		printf("dCov2D_dtau idx %d  dc/dtau: %f %f %f %f %f %f\n", idx, dcov2D_dtau[2][0], dcov2D_dtau[2][1], dcov2D_dtau[2][2], dcov2D_dtau[2][3], dcov2D_dtau[2][4], dcov2D_dtau[2][5]);
	}
	...
	float *dL_dtau)
{
	...
	{
		// Print dmean2D_dtau for debugging
		// printf("idx %d, dmean2D_dtau:\n", idx);
		printf(" dmean2D_dtau idx %d  (%f, %f, %f, %f, %f, %f, )\n", idx, dmean2D_dtau[0].x, dmean2D_dtau[1].x, dmean2D_dtau[2].x, dmean2D_dtau[3].x, dmean2D_dtau[4].x, dmean2D_dtau[5].x);	
		printf(" dmean2D_dtau idx %d  (%f, %f, %f, %f, %f, %f, )\n", idx, dmean2D_dtau[0].y, dmean2D_dtau[1].y, dmean2D_dtau[2].y, dmean2D_dtau[3].y, dmean2D_dtau[4].y, dmean2D_dtau[5].y);
	}
