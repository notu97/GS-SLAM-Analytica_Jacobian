...
__global__ void preprocessCUDA(int P, int D, int M,
	const float* orig_points, // Gaussian means in 3D world coords
	...
	bool prefiltered)
{
	...
	{	
		printf("Using precomputed cov3D for idx %d\n", idx);
		cov3D = cov3D_precomp + idx * 6;
	}
	else
	{
		const float* cov3D_before = cov3Ds + idx * 6;
		...
		if (idx == 29181) {
			printf("Computing cov3D for idx %d scale %f %f %f, scale_modifier %f, rotation %f %f %f %f\n", idx, scales[idx].x, scales[idx].y, scales[idx].z, scale_modifier, rotations[idx].x, rotations[idx].y, rotations[idx].z, rotations[idx].w);
			glm::vec3 scale = scales[idx]; 
			float mod = scale_modifier; 
	...
	if( idx == 29181 ) {
		printf("idx: %d, p_orig: %f, %f, %f\n", idx, p_orig.x, p_orig.y, p_orig.z);
		printf("idx: %d, Cov3D: %f, %f, %f, %f, %f, %f\n", idx, cov3D[0], cov3D[1], cov3D[2], cov3D[3], cov3D[4], cov3D[5]);
		printf("idx: %d, focal_x: %f, focal_y: %f, tan_fovx: %f, tan_fovy: %f\n", idx, focal_x, focal_y, tan_fovx, tan_fovy);
		printf("idx: %d, Cov2D before adjustment: %f, %f, %f\n", idx, cov.x, cov.y, cov.z);
	...
	if (colors_precomp == nullptr)
	{
		...
		if( idx == 29181 ) {
			printf("Computed Color from SH for idx %d: %f, %f, %f\n", idx, result.x, result.y, result.z);
		}
	}
